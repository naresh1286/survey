								...
								<%-- ===========================${question.inputType.inputTypeName}============================<br/> --%>
									...
									<div id="q${qi.index+1}" class="${qi.index == 0 ? 'show' : 'hide'}">
										...
										<c:forEach var="questionAnswer" items="${questionAnswerForm.questionAnswers}" varStatus="qai">											
											...
											<div class="${question.inputType.inputTypeName == 'single' ? 'checkbox' : 'radio'}">
												<label><input type="${question.inputType.inputTypeName == 'single' ? 'checkbox' : 'radio'}" id="q${qi.index+1}o" name="q${qi.index+1}o" onclick="loadRelativeQuestion('${questionAnswer.relativeQuestionId}','${questionAnswer.conditionQuestionId}','${qi.index+1}','${ai}')" value="${questionAnswer.answer.answerId}">${questionAnswer.answer.answerText}</label>
											</div>
